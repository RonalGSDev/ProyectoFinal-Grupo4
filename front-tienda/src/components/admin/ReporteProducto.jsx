...
const ReporteProducto = () => {
  ...
  const fetchPedidos = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8080/pedidos/obtener?productoid=${productoId}`);
        ...
        <div className='col table-responsive'>
          <table className="table table-hover table-striped text-center">
            ...
            <tbody>
              {pedidos.map((pedido) => (
                <tr key={pedido[0]}>
                  <td>{pedido[0]}</td> {/* ID Pedido */}
                  <td>{pedido[1]}</td> {/* ID Cliente */}
                  <td>{pedido[2]}</td> {/* Nombres y Apellidos */}
                  <td>{pedido[3]}</td> {/* Fecha */}
                  <td>{pedido[4]}</td> {/* Cantidad de Productos */}
                  <td>{pedido[5].toFixed(2)}</td> {/* Total */}
                  <td>
